...
class GraphWithAdjacencyList {
	...
	addVertex(vertex) {
		// 이미 존재하는 정점이라면 덮어씌워지지 않게 방지한다.
		this.vertices[vertex] = this.vertices[vertex] || [];
	}

	contains(vertex) {
		// 인자로 넘겨받은 정점의 존재여부를 반환한다.
	...
	addEdge(fromVertex, toVertex) {
		...
		// 두번째 정점의 인접 리스트에 첫번째 정점을 추가한다. (간선이 존재하지 않을 경우)
	...
	hasEdge(fromVertex, toVertex) {
		...
		// 존재한다면 해당 정점의 리스트에 toVertex가 포함되어있는지 반환한다.
