...
export const Autocomplete = () => {
  ...
  const handleKeyUp = (event) => {
    ...
    if (hasText) {
      if (event.code === 'ArrowDown' && options.length - 1 > selected) {  // 화살표가 아래로 갈 때
        setSelected(selected + 1);
      }
      if (event.code === 'ArrowUp' && selected >= 0) {  // 화살표가 위로 갈때 && 인덱스가 0보다 더 작아지지 않게 방지
        setSelected(selected - 1);
      }
      if (event.code === 'Enter' && selected >= 0) {  // 엔터를 치면
    ...
    <DropDownContainer>
      {options.map((option, idx) => (
        <li
          ...
          className={selected === idx ? 'selected' : ''}  // selected는 인덱스 값을 가리켜서 선택된 li에 클래스명을 줌
        >
          {option}
        </li>
      ))}
