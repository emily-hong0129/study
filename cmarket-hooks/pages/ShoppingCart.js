...
export default function ShoppingCart({ items, cartItems, handleDeleteCartItems, changeQuantity}) {
  const [checkedItems, setCheckedItems] = useState(cartItems.map((el) => el.itemId))

  // 개별선택
  const handleCheckChange = (checked, id) => {
    if (checked) {  // 체크했을때
      setCheckedItems([...checkedItems, id]); // [1, 5, 2] + 3 => [1, 5, 2, 3]
    }
    else {  // 체크해제 
      setCheckedItems(checkedItems.filter((el) => el !== id));  // [1, 5, 2]  3를 제외
    }
  };
  ...
  const handleQuantityChange = (quantity, itemId) => {
    changeQuantity(quantity, itemId)
  }
